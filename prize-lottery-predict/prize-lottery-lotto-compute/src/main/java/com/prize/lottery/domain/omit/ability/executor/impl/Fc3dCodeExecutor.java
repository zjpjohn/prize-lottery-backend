...
public class Fc3dCodeExecutor implements CodeExecutor {
    ...
    public void initialize(Integer limit) {
        ...
        List<LotteryCodeDo> codeList = lotteries.stream().flatMap(lottery -> {
            return LotteryCodeDo.fc3d(lottery.getPeriod(), lottery.getRed()).stream();
    ...
    public void execute(String period) {
        Optional.of(mapper.getLotteryInfo(this.bizIndex().getType(), period))
                .map(lottery -> LotteryCodeDo.fc3d(period, lottery.getRed()))
