...
import com.prize.lottery.domain.omit.ability.LotteryDanAbility;
...
public class CalcLotteryDanHandler implements EventHandler<CalculatorEvent> {
    ...
    public void onEvent(CalculatorEvent event, long sequence, boolean endOfBatch) throws Exception {
        ...
        try {
            lotteryDanAbility.calculate(type, period);
        } catch (Exception error) {
            log.error("计算{}第{}期胆码遗漏错误:", type.getNameZh(), period, error);
        }
    }
}
