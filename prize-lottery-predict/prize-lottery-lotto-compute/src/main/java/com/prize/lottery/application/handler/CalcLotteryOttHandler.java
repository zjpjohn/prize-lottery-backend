...
import com.prize.lottery.domain.omit.ability.LotteryOttAbility;
...
public class CalcLotteryOttHandler implements EventHandler<CalculatorEvent> {
    ...
    public void onEvent(CalculatorEvent event, long sequence, boolean endOfBatch) throws Exception {
        ...
        try {
            lotteryOttAbility.execute(type, period);
        } catch (Exception error) {
            log.error("计算{}第{}期012路遗漏错误:", type.getNameZh(), period, error);
        }
    }
}
