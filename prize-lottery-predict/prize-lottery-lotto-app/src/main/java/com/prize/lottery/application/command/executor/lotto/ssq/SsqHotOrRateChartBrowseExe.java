...
public class SsqHotOrRateChartBrowseExe {
     ...
     */
    public List<SsqLottoCensusPo> execute(Long userId, ChartType chartType) {
    ...
    public FeeDataResult<SsqChartCensusVo> executeV1(Long userId, ChartType chartType) {
        Period period = browseForecastRepository.latestChartPeriod(chartType);
        Assert.notNull(period, ResponseHandler.FORECAST_NONE);

        ForecastBrowse browse = new ForecastBrowse(userId, period, LotteryEnum.SSQ);
        browse.lottoChart(chartType, properties.getExpend());
        Pair<Boolean, List<SsqLottoCensusPo>> executed =
            browseExecutor.executeNull(browse, () -> browseForecastRepository.getForecastHotOrRateChart(period, chartType));
        if (executed.getKey()) {
