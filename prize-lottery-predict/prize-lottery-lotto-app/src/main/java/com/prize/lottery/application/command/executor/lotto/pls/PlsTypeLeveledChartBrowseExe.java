...
public class PlsTypeLeveledChartBrowseExe {
     ...
     * @param level     分级级别
     */
    public List<Pl3LottoCensusPo> execute(Long userId, ChartType chartType, Integer level) {
     ...
     * 统一返回V1版本数据格式
     */
    public FeeDataResult<NumberThreeCensusVo> executeV1(Long userId, ChartType chartType, Integer level) {
        Period period = browseForecastRepository.latestChartPeriod(chartType);
        Assert.notNull(period, ResponseHandler.FORECAST_NONE);

        ForecastBrowse browse = new ForecastBrowse(userId, period, LotteryEnum.PL3);
        browse.lottoChart(chartType, properties.getExpend());
        Pair<Boolean, List<Pl3LottoCensusPo>> executed = browseExecutor.executeNull(browse, () -> browseForecastRepository.getTypLevelFullOrVipChart(chartType, period, level));
        if (executed.getKey()) {
