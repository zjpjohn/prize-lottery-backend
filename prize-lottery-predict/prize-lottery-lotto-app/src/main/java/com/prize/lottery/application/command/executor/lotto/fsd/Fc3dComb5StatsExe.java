...
public class Fc3dComb5StatsExe {
    ...
    public N3Comb5StatsVo execute(String period,
        ...
        List<Fc3dIcaiDataVo> dataList = mapper.getFc3dRankedDataList(Fc3dChannel.COMB5.getChannel(), period, last, 20);
        if (CollectionUtils.isEmpty(dataList)) {
            return null;
        }
        Boolean contain = Optional.ofNullable(containZu3).orElse(true);
        List<N3Comb5StatsVo.StatsInfo> statsList = dataList.stream()
                                                           .map(Fc3dIcaiPo::getComb5)
                                                           .map(ForecastValue::getData)
                                                           .flatMap(e -> N3Comb5StatsVo.flatLottery(e, contain))
