...
public class QlcTypeLevelChartBrowseExe {
     ...
     * @param level     数据分级
     */
    public List<QlcLottoCensusPo> execute(Long userId, ChartType chartType, Integer level) {
    ...
    public FeeDataResult<QlcChartCensusVo> executeV1(Long userId, ChartType chartType, Integer level) {
        Period period = browseForecastRepository.latestChartPeriod(chartType);
        Assert.notNull(period, ResponseHandler.FORECAST_NONE);

        ForecastBrowse browse = new ForecastBrowse(userId, period, LotteryEnum.QLC);
        browse.lottoChart(chartType, properties.getExpend());
        Pair<Boolean, List<QlcLottoCensusPo>> executed = browseExecutor.executeNull(browse, () -> browseForecastRepository.getTypLevelFullOrVipChart(chartType, period, level));
        if (executed.getKey()) {
